...
export default function useNetlifyIdentity(onAuthChange) {
  ...
  const itemChangeCallback = _user => {
    if (_user) {
      console.log({ _user })
      const faunadb_token = _user && _user.app_metadata && _user.app_metadata.faunadb_token
  ...
  const [item, setItem, removeItem] = useLocalStorage("faunaNetlifyUser", itemChangeCallback)
  React.useEffect(() => {
